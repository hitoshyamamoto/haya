...
describe('SQL Databases Specific Tests', () => {
  ...
  describe('SQLite Database', () => {
    test('should create and use SQLite database', async () => {
      ...
      expect(versionResult).toContain('3.');
    }, testTimeout);
  ...
  describe('DuckDB Database', () => {
    test('should create and use DuckDB database', async () => {
      ...
      expect(versionResult).toContain('1.');
    }, testTimeout);
    ...
    test('should compare performance characteristics', async () => {
      ...
      for (const engine of sqlEngines) {
        ...
        if (engine === 'postgresql') {
          ...
          await execAsync(`docker exec ${instanceName} psql -U admin -d database -c "INSERT INTO perf_test (data) SELECT 'test' FROM generate_series(1, 100);"`);
          ...
          const insertPromises = Array(100).fill(0).map(() => 
            execAsync(`docker exec ${instanceName} mysql -u admin -ppassword -e "INSERT INTO database.perf_test (data) VALUES ('test');"`));
